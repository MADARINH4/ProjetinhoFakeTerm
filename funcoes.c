...
void imprimeTela(int tentativa, int tentativasMax, int letras, char listaChutes[][letras], char listaResultados[][letras]){
  ...
  for (int c = 0; c < tentativasMax; c++){
    ...
    if (c==0) printf("\t     __________L  E  G  E  N  D  A____________ ");
    if (c==1) printf("\t    /                                         \\ ");
    if (c==2) printf("\t    | \"[a]\" - contem a letra nesta posicao.   |");
    if (c==3) printf("\t    | \"<a>\" - contem a letra noutra posicao.  |");
    if (c==4) printf("\t    | \" a \" - nao contem a letra.             |");
    if (c==5) printf("\t    \\_________________________________________/");
    printf("\n"); //quebra de linha
  ...
  if(tentativa == tentativasMax && contaLetrasCorretas != letras - 1){
